...
class mobileWrapper
{
	...
	{		
		...
		$path_to_upload="https://www.grubhouse.co.uk/upload/".$addon_path;
		
		if (empty($image)){
			$image=Yii::app()->functions->getOptionAdmin('mobile_default_image_not_available');
		}					
		
		$default_image ="https://www.grubhouse.co.uk/protected/modules/".APP_FOLDER."/assets/images/$default";	
		if(!empty($image_set)){
			$default_image = "https://www.grubhouse.co.uk/protected/modules/".APP_FOLDER."/assets/images/$image_set";	
		}
		...
		if (!empty($image)){			
			if (file_exists($path_to_upload."/$image")){							
				$default=$image;							
				if(empty($addon_path)){
				  $url = "https://www.grubhouse.co.uk/upload/$default";
				} else {
				   $url = "https://www.grubhouse.co.uk/upload/$addon_path/$default";	
				}
			...
			foreach ($res as $val) {
				...
				if(isset($val['applicable_to'])){
    			   ...
    			   if(is_array($applicable_to) && count($applicable_to)>=1){
    			   	  foreach ($applicable_to as $applicable_to_val) {    			   	  	 
							  ...
							  $applicable_to_text.=t($applicable_to_val).",";
    			   	  }
				   }  
				   $applicable_to_text = substr($applicable_to_text,0,-1);  			
    			}    		 
    			...
    			if (!empty($applicable_to_list)){    				
	    			$offer = self::t("[percent]% Off over [amount] if [transaction]",array(
	    			  ...
	    			  '[transaction]'=>$applicable_to_text
	    			));
    			} else {	    			
    			...
    			$offer_list[] =array(
					'rawPercent'=>number_format($val['offer_percentage'],0),
					'rawOver'=>number_format($val['offer_price'],2),
					'available'=>$applicable_to_list,
    			   	'full'=>$offer
    			);
			}
			return $offer_list;
...
/*	public static function prettySort($sort_by = '')
    ...
    public static function getCart($device_uiid='', $merchant_id='')
    {
    	if(empty($device_uiid)){
    		return false;
    	}
    ...
    public static function clearCart($device_uiid='', $merchant_id='')
    {    	
    	if(empty($device_uiid)){
    		return false;
    	}
     ...
     public static function getAddressBookByClient($client_id='')
    {
    	...
    	$stmt="SELECT      	       
    	       ...
    	       id,location_name,country_code,latitude, longitude, as_default
    	       FROM
    	       {{address_book}}
    	       WHERE

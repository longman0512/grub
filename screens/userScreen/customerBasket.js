...
import { RemoveItem, LoadCart, GetCartCount, AddDeliveryAddress, GetRecentAddress } from "../../utils/Api"
...
class customerBasket extends Component {
  ...
  calculateTotal=(TipFlag)=>{
    ...
    console.warn(this.total, "total in calculateTotal func")

    var max = 0
    var  flag = false
    console.log(this.state.offers, this.state.discount, "discount and offers")
    ...
    if(this.state.discount.discount_type == "fixed amount"){
      if((this.total - discount_amount) >= 3){
        ...
        // this.setState({appliedDiscount: [...this.state.appliedDiscount, {
        //   name: "discount",
      ...
      this.state.offers.map((item, index)=>{
        if(item.rawPercent != 100){
          if(item.available.indexOf(this.state.services[this.state.selectedIndex].toLowerCase())>=0){
            // if(this.state.offers[max].available.indexOf(this.state.services[this.state.selectedIndex].toLowerCase())>=0){
              if(!flag && this.total >= Number(item.rawOver)){
                max = index
                flag = true
                // this.setState({appliedDiscount: [...this.state.appliedDiscount, {
                //   name: item.full,
                //   amount: item.rawPercent+"%"
                // }]})
                this.setState({appliedDiscount: this.state.appliedDiscount + this.total * item.rawPercent/100})
              }
    ...
    if(flag){
      if(typeof this.state.offers[max]!= "undefined"){
        console.log(this.state.offers[max].rawOver, "max offers to optimized you")
        this.total = this.total * (100-this.state.offers[max].rawPercent)/100
      }
    }
    console.log(this.state.offers[max].rawPercent, this.total, "max offer applied")
    if(TipFlag){
      console.log("TIP valide")
      this.total = Number(this.total+0.5+Number(this.state.merchant_info.delivery_charges)+Number(this.state.tip_more)+Number(this.state.res_tip))

    } else {
      console.log("TIP disabled")
      this.total = Number(this.total+0.5+Number(this.state.merchant_info.delivery_charges))
    }
  ...
  toggleTipSwitch = (val) => {
    ...
    }, this.calculateTotal(this.state.switchTipValue?false:true));
    
  };
  updateIndex =()=>{
    this.setState({selectedIndex: this.state.selectedIndex==0?1:0}, ()=>{
      console.log(this.state.services[this.state.selectedIndex])
  ...
  const DATA = await Axios({
    ...
    function (response) {
      if(response.data.details){
        ...
        console.log("============================================================")
        console.log(this.state.cart.cart.data.total.total, Number(response.data.details.delivery_charges), (this.state.cart.cart.data.total.total+Number(response.data.details.delivery_charges)+0.5))
        console.log(response.data.details, "discounts results")
        if(total>=0.5){
          this.setState({res_tip: 1-total})
        }  else if(total == 0) {
          this.setState({res_tip: 0})
        } else {
          this.setState({res_tip: 0.5-total})
        }
  ...
  //   if (this.state.polylinePath.length < this.myLatLongs.length) {
  //      const Direction = this.myLatLongs;
  //      const polylinePath = [
...
//  };
  ...
  addDeliveryAddress =async ()=>{
    ...
    if(Platform.OS == "ios"){
      ...
      await Axios({
        method: "get",
        url: "https://maps.googleapis.com/maps/api/geocode/json?components=postal_code:"+this.state.postCode+"|country:UK&key=AIzaSyCej2vLb-XXyKoWeMzdAUynqZbq0YVmWi0",
      }).then(res=>{
        console.log(res.data.results)
        var temp = []
        if(res.data.results.length == 0){
      ...
      await Axios({
        method: "get",
        url: "https://maps.googleapis.com/maps/api/geocode/json?components=postal_code:"+this.state.postCode+"|country:UK&key=AIzaSyCCynf5qQzLMr2CLR0sWWLgsq6vT8ad4M0",
      }).then(res=>{
        console.log(res.data.results)
        var temp = []
        res.data.results.map((add, index)=>{
  ...
  selectDate = ()=>{
    ...
    var openTime = this.state.merchant_info.stores_open_starts[this.dateString[dayWeek]]
    var closeTime = this.state.merchant_info.stores_open_ends[this.dateString[dayWeek]]
    if(openTime){
      openTime = new Date(this.year, this.month, this.date, openTime.split(":")[0], openTime.split(":")[1])
      closeTime = new Date(this.year, this.month, this.date, closeTime.split(":")[0], closeTime.split(":")[1])
      var sOpenTime = new Date(this.year, this.month, this.date, selectedTime[0].split(":")[0], selectedTime[0].split(":")[1])
      var cOpenTime = new Date(this.year, this.month, this.date, selectedTime[1].split(":")[0], selectedTime[1].split(":")[1])
      if(sOpenTime < openTime || cOpenTime > closeTime){
        console.log("it is not valid")
        Alert.alert("GRUBHOUSE", "Please select another time, At that day we are open from "+this.state.merchant_info.stores_open_starts[this.dateString[dayWeek]]+" to "+this.state.merchant_info.stores_open_ends[this.dateString[dayWeek]])
      } else {
  ...
  reSelectTime = ()=>{
    ...
    if(openTime){
      ...
      var temp = []
      for(var i =Number(openTime.split(":")[0]); i<=closeTime.split(":")[0]; i++){
        if(i<10){
          temp.push("0"+i+":00-0"+i+":30")
          if((i+1>9)){
            temp.push("0"+i+":30-"+(i+1)+":00")
              ...
              <View  style={{width: "90%", height: 60, flexDirection: "row", justifyContent: "flex-start", alignItems: "center", position: "relative"}}>
                ...
                >
                  {/* <TextInput 
                    ...
                    onChangeText={(v) => {
                      this.setState({filterDAtext: v.replace(" ", "")})
                      if(v ==""){
                        this.setState({filteredAddress: []})
                      }
                      this.filterDeliveryAddress()
                    }}
                    style={{width:"90%", height: 50, fontSize:16}}
                    // value={this.state.filterDAtext}
                  /> */}
                  {/* <Text style={{color: "gray", fontSize: 16}}>London</Text> */}
                  <GooglePlacesAutocomplete
                    ...
                    query={{
                      key: Platform.OS == "ios"?"AIzaSyCej2vLb-XXyKoWeMzdAUynqZbq0YVmWi0":"AIzaSyCCynf5qQzLMr2CLR0sWWLgsq6vT8ad4M0",
                      language: 'en',
          ...
          <MapView
            ...
            >
              ...
              {/* <MapViewDirections 
                ...
                strokeColor="black"
                // origin={{
                //   latitude: Number(global.LOCATION.latitude?global.LOCATION.latitude:0),
                //   longitude: Number(global.LOCATION.longitude?global.LOCATION.longitude:0),
                // }} 
                origin={this.state.user_info} /> */}
              {/* <AnimatedPolyline coordinates = {[...this.animPolyCoords]} interval={10} /> */}
              {/* <Polyline
                strokeWidth={3}
                strokeColor={"black"}
                coordinates={[...this.state.myLatLongs]}
              /> */}
            </MapView>
              ...
              {/* {this.state.schedulePicker &&
                <DatePicker
                  ...
                  onDateChange={(date) => {
                    ...
                    this.setState({ date: date, schedulePicker: false });
                  }}
                ...
                />} */}
                {/* <DateTimePickerModal
                  isVisible={this.state.schedulePicker}
                  mode="date"
                  onConfirm={(v)=>{
                    console.log("ok", new Date(v).toDateString())
                    var Sdate = new Date(v).toDateString()
                    this.setState({schedulePicker: false, date: Sdate})
                    this.Timpicker.open()
                  }}
                  onCancel={()=>{
                    console.log("cancel")
            ...
            >
              ...
              />
              <View>
                <Text style={{fontSize: 18}}>Support your local restaurant</Text>
                <Text style={{fontSize: 14, color: "gray"}}>Round up your bill or add a bit extra</Text>
              </View>
            ...
            >
              ...
              <View style={{flexDirection: "row", justifyContent: "center", alignItems: "center"}}>
                <Text style={{fontSize: 18}}>£{this.state.res_tip.toFixed(2)}</Text>
                <Switch
            ...
            <View
              style={styles.rowContainer}
            >
              <Text style={{fontSize: 18, color: "gray"}}>Tip more</Text>
              <View style={{flexDirection: "row", justifyContent: "center", alignItems: "center"}}>
                <Text style={{fontSize: 18}}>£{this.state.tip_more}</Text>
              </View>
            ...
            >
              ...
              >
                <Card
                  style={{width: 100, margin: 10}}
                >
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.1)}}>10%</Text>
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.1)}}>+ Add</Text>
                </Card>
                <Card
                  style={{width: 100, margin: 10}}
                >
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.15)}}>15%</Text>
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.15)}}>+ Add</Text>
                </Card>
                <Card
                  style={{width: 100, margin: 10}}
                >
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.2)}}>20%</Text>
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.2)}}>+ Add</Text>
                </Card>
                <Card
                  style={{width: 100, margin: 10}}
                >
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.25)}}>25%</Text>
                  <Text style={{fontSize: 18, color: "gray", textAlign: "center"}} onPress={()=>{this.calculatorTip(0.25)}}>+ Add</Text>
                </Card>
              </ScrollView>
            </View>
            <Divider style={styles.divider} />
            ...
            >
              ...
              <View style={styles.tbRowContainer}>
                ...
                <View style={{width: "20%"}}>
                  <Text style={{textAlign: "right"}}>£{Number(this.state.merchant_info.delivery_charges).toFixed(2)}</Text>
                </View>
              </View>
              <View style={styles.tbRowContainer}>
                <View style={{width: "30%"}}>

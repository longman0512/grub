...
import { SearchCuisineById, StoreUserData, GetUserData } from "../../utils/Api";
...
export default class nearbyRestorants extends Component {
  ...
  getRests = async (location) => {
    ...
    var city = ''
    // if (Platform.OS === "ios") {
      var address = await Location.reverseGeocodeAsync(location)
    ...
    //     street = json.results[0].address_components[0].long_name + " " + json.results[0].address_components[1].long_name;
    //     city = json.results[0].address_components[2].long_name;
    //     let data = []
    //     this.setState({ cAddress: city })
    //     getNearBy(location)
    //       .then(response => {
  ...
  sendMail = () => {
    if(this.state.userEmail==""){
      ...
      MailComposer.composeAsync({
        subject: 'I want to suggest this location',
        body: "My Email " + this.state.userEmail + "\n<br/> And I suggest this address:" + this.state.suggestAddress,
